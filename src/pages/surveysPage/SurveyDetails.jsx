import { Card, Label, Radio, Spinner } from 'flowbite-react';
...
const SurveyDetails = () => {
 ...
 const handleVote = e => {
  ...
  const myData = { vote, surveyFeedBack, title, descriptions, category, status, commentText, report };
  console.log(myData);
  axiosPublic
 ...
 return (
  <form onSubmit={handleVote}>
   <Card className="max-w-sm" horizontal>
    ...
    <hr />
    <p className="font-medium text-gray-700 dark:text-gray-400">{descriptions}</p>
    <hr />
    {/* vote */}
    ...
    <label className="capitalize">if u Do not like this survey tell us about it!</label>
    <input type="text" name="report" />
    <button disabled={usersData?.role === 'admin' || usersData?.role === 'surveyor' || filter?.surveyId === _id} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded disabled:bg-gray-500">
     Submit
    </button>
   </Card>
  </form>
